...
8. `supabase/homework_materials.sql` — teacher homework material files (`homework-materials` public bucket) + `homeworks.material_*` columns
9. `supabase/schedule.sql` — weekly `schedule_slots` per group + RLS
10. `supabase/remarks.sql` — teacher remarks/praise (`remarks`) with history + RLS
11. `supabase/demo_seed_role_data.sql` (optional demo data)

> Or just run the single consolidated file `supabase/_setup_all.sql` (all of
> the above except the optional demo seed, concatenated in order).
